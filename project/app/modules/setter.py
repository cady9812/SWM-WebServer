...
def product_command(src_ip, dst_ip, attack_id_list):
    ...
    for attack_id in attack_id_list:
        dst_port, usage, type = Attack.query.filter(Attack.attackId==attack_id).with_entities(Attack.port, Attack.usage, Attack.type).first()
        if type=="malware":
            chk = True
        else:
            chk = False
...
def malware_command(src_ip, attack_id_list):
    command = []
    for attack_id in attack_id_list:
        usage = Attack.query.filter(Attack.attackId==attack_id).with_entities(Attack.port, Attack.usage).first()
        file_route = f"{downloadURL}/{attack_id}"
        command.append({
            "type":"malware",
            "src_ip":src_ip,
